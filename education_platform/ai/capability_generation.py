...
## 正式图谱复用规则
1. 你的输出是“正式图谱的增量候选”，不是重新生成完整能力图谱。正式图谱已经覆盖的内容不要输出。
...
4. 只有正式图谱没有、且招聘要求有明确新证据支持的内容，才可以作为新节点输出。
5. 同一含义只输出一次，不得同时输出正式名称及其同义改写。
6. 如果新招聘要求已经被正式图谱完全覆盖，必须输出空的 abilities 数组；零新增是正确结果，不得为了凑数量创造节点。
7. 新知识点挂在正式能力或正式能力单元下面时，可以输出该正式父路径，但父节点名称必须逐字复用正式名称；父路径只是定位信息，不代表它本身是新增节点。

## 生成标准
1. 只提取招聘要求中有证据支持的内容；没有依据的设备、软件、标准和证书不要补写。
2. 最多输出 3 个包含真实增量的岗位能力；没有增量时输出 0 个。每个能力都必须至少包含一个正式图谱尚未覆盖的新知识点/技能点。
3. 能力名称使用“对象 + 工作任务”，例如“数控机床操作”“工件装夹与找正”“加工质量检测”，不要使用“综合能力”“相关技能”“熟悉设备”等空泛名称。
4. 全部输出最多包含 8 个新增知识点/技能点，不设最低数量。每个点只能描述一个可观察、可考核的动作，不能用“并且、同时、以及”串联多个动作。
...
9. 忽略福利待遇、学历、工作年限、加班倒班、团队精神等非能力信息，也忽略明显不属于{job_name}岗位的误采集职位内容。
10. 判断“新增”时比较语义而不是字面：设备型号、软件品牌、示例材料、括号说明、精度数值不同，若核心操作动作相同，仍视为正式图谱已覆盖。
