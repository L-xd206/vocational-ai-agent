...
def create_analysis_batch(job, tree, *, crawl_task=None, raw_ai_output="", model_name="", batch=None):
    ...
    for ai, ability_data in enumerate(normalized_tree):
        college = _college_from_name(ability_data.get("college"))
        ability_candidate, ability_match = add_candidate(
            ability_data, "ability", None, None, ai, college
        )
        for ui, unit_data in enumerate(ability_data.get("units", [])):
            unit_candidate, unit_match = add_candidate(
                unit_data, "unit", ability_candidate, ability_match, ui
            )
            for pi, point_data in enumerate(unit_data.get("children", [])):
                add_candidate(point_data, "point", unit_candidate, unit_match, pi)
    return batch
